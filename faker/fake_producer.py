...
RATE = int(os.getenv("RATE", "5"))  # rows per sec. defaults to low speed for debug
PAYLOAD = int(os.getenv("PAYLOAD", "600"))
...
def build_payload(i):
    ...
    proto = json.dumps(
        payload, default=lambda o: int(o) if isinstance(o, np.integer) else o
    )  # or maybe just use plain int from the start?
    ...
    while True:
        ...
        while sent < should_have_sent:
            ...
            p.produce(
                TOPIC,
                build_payload(sent),
                callback=lambda err, msg, t0=t0: dr_cb(err, msg, t0),
            )
            prod_msgs.inc()
